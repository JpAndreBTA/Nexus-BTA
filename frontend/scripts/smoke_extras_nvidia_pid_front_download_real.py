...
def visual_metrics(path: Path) -> dict[str, object]:
    ...
    frames_dir = RESULTS / f"frames_extras_nvidia_pid{SUFFIX}"
    ...
    if repeated > 0.65:
        raise AssertionError(f"NVIDIA PiD Extras output appears mostly frozen/repeated: {consecutive}")
    if float(np.mean(local_diff)) > 55:
        raise AssertionError(f"NVIDIA PiD Extras output looks noise-heavy: local diff {float(np.mean(local_diff)):.2f}")
    return {
        "stream": stream,
...
def make_contact_sheet(path: Path) -> Path:
    sheet = RESULTS / f"front_extras_nvidia_pid_real{SUFFIX}_strip.jpg"
    subprocess.run(
        [
            "ffmpeg",
            "-y",
...
def metadata_for(output_path: Path) -> dict[str, object]:
    ...
    if not meta_path.exists():
        raise AssertionError(f"Missing metadata sidecar: {meta_path}")
...
def main() -> None:
    ...
    with sync_playwright() as p:
        ...
        page.evaluate(
            ...
            async () => {
              ...
              document.querySelector('#extrasVideoUpscaleEngine').value = 'nvidia_pid';
              document.querySelector('#extrasPidProfileSelect').value = 'lowvram_zimage_2k';
              ...
              syncExtrasAlphaUi();
              syncExtrasEncodeUi();
              return await maybePrepareNvidiaExtrasEngine('nvidia_pid');
            }
            """ % ("true" if REFINE else "false", "true" if REFINE else "false", "true" if FACE else "false", "true" if FACE else "false")
        )
        page.wait_for_function("() => document.querySelector('#extrasPidStatusText')?.innerText?.toLowerCase().includes('ready')", timeout=1800000)
        ...
        page.screenshot(path=str(RESULTS / f"front_extras_nvidia_pid_real{SUFFIX}.png"), full_page=True)
        browser.close()
    ...
    if upscale.get("engine") != "nvidia_pid" or upscale.get("pid_prepared") is not True:
        raise AssertionError(f"Expected prepared NVIDIA PiD metadata, got {upscale!r}")
    sheet = make_contact_sheet(output_path)
    result = {"output": str(output_path), "url": output_url, "contact_sheet": str(sheet), "metrics": metrics, "upscale": upscale}
    (RESULTS / f"front_extras_nvidia_pid_real{SUFFIX}.json").write_text(json.dumps(result, indent=2), encoding="utf-8")
    print(f"ok extras NVIDIA PiD front download battery: {output_path}")
