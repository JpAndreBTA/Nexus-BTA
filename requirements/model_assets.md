...
- `run.bat`, `update.bat`, and `scripts/bootstrap_nexus_runtime.ps1` run `scripts/install_wan22_deps.ps1`, which verifies Wan 2.2 shared support files in `models/text_encoders`, `models/vae`, and `models/clip_vision`. Pass `-IncludeWanModels` to that script only when you also want the large FP8 high/low Wan model pair downloaded to `models/diffusion_models`.
...
- LTX 2.3 video-to-video motion transfer uses IC-LoRA Union Control when available. `run.bat`, `update.bat`, and `scripts/install_ltx_director_deps.ps1` verify `models/loras/ltx_ic/ltx-2.3-22b-ic-lora-union-control-ref0.5.safetensors`; Nexus also auto-detects copies under `models/loras/ltx` before fallback IC-LoRAs such as Cameraman.
