...
- Ideogram 4 regional guide boxes are converted to JSON `compositional_deconstruction.elements[]` entries with normalized `[y_min, x_min, y_max, x_max]` bboxes. Use 512x512, 4 steps and CFG 1 for smoke batteries; increase to 8-12 steps only when the selected workflow/model needs it.
