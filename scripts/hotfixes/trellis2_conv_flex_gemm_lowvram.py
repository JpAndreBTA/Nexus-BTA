...
def sparse_conv3d_forward(self, x: SparseTensor) -> SparseTensor:
    # NEXUS_TRELLIS_FLEX_GEMM_LOWVRAM_HOTFIX
    ...
    if os.environ.get("SPARSE_DEBUG") == "1":
        neighbor_mb = (N * V * 4) / 1024 / 1024
        output_mb = (N * Co * x.feats.element_size()) / 1024 / 1024
        feats_mb = (x.feats.numel() * x.feats.element_size()) / 1024 / 1024
        print(f"[CONV] N={N} Ci={Ci} Co={Co} neighbor_map={neighbor_mb:.0f}MB output={output_mb:.0f}MB feats={feats_mb:.0f}MB")
    ...
    if neighbor_cache is None:
        ...
        neighbor_map_bytes = x.feats.shape[0] * V * 4
    ...
    original_feats = x.feats
    feats = x.feats
    N = feats.shape[0]
    weight_mat = self.weight.reshape(Co, V * Ci).t()
    im2col_bytes_per_voxel = V * Ci * feats.element_size()
    chunk_cap_mb = int(os.environ.get("NEXUS_TRELLIS_FLEX_GEMM_CHUNK_MB", "64"))
    chunk_cap = max(16, chunk_cap_mb) * 1024 * 1024
    chunk = max(1024, chunk_cap // im2col_bytes_per_voxel)

    output_bytes = N * Co * feats.element_size()
    output_offload_mb = int(os.environ.get("NEXUS_TRELLIS_FLEX_GEMM_OFFLOAD_OUTPUT_MB", "256"))
    offload_output = output_bytes >= max(128, output_offload_mb) * 1024 * 1024
    if offload_output:
    ...
    for start in range(0, N, chunk):
        ...
        chunk_map = neighbor_map[start:end]
        if stream_from_cpu:
